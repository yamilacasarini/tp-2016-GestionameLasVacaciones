...
IF EXISTS (select * from sys.objects where type = 'TR' and name = 'GESTIONAME_LAS_VACACIONES.seModificoPlan')
...
RETURNS TABLE AS 
RETURN (SELECT TOP 5 especialidad.id, GESTIONAME_LAS_VACACIONES.getDescEspecialidad(especialidad.id) as especialidad FROM GESTIONAME_LAS_VACACIONES.Especialidades especialidad
